...
if($_SESSION['em_id'] != ''){
    ...
    $products = "SELECT * FROM tb_product ORDER BY product_ID DESC ";
    $qproduct = mysqli_query($sol, $products);
    $producttomers = mysqli_fetch_all($qproduct, MYSQLI_ASSOC);
    echo json_encode($producttomers);
    // ปิดการเชื่อมต่อ
    mysqli_close($sol);
} else {
