...
if($_SESSION['em_id'] != ''){
    ...
    if ($_SESSION["ext"] == 'IT2' OR  $_SESSION["ext"] == 'PRM') {
        $cuss = "SELECT distinct customer_name FROM tb_customer_hos WHERE customer_name != '' ORDER BY customer_name ASC ";
        ...
        $cuss = "SELECT distinct customer_name FROM tb_customer_hos WHERE customer_name != '' AND sale_area IN ('".$em_idFull."') ORDER BY customer_name ASC ";
    } else {
        $em_idFull = $_SESSION['em_id'];
        $cuss = "SELECT distinct customer_name FROM tb_customer_hos WHERE customer_name != '' AND sale_area = '".$em_idFull."' ORDER BY customer_name ASC ";
    }
        // echo $cuss;
