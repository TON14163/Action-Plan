...
if (!empty($_REQUEST['id_work'])) {
    ...
    if (!empty(($_REQUEST['dc']))) {
        $dc = $_REQUEST['dc'];
        if ($dc == '1') {
            $plan_copynew = $_REQUEST['plan_copynew'];
...
<div style="position: fixed; top:30px; right: 20px; " class="alert alert-success alert-dismissible fade show"
    role="alert"><img src="assets/images/icon_system/swap-horizontal.png" style="width: 20px; height: 20px;"> สลับ Plan
    แล้ว<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>
<script>
setTimeout(function() {
...
<div style="position: fixed; top:30px; right: 20px; " class="alert alert-success alert-dismissible fade show"
    ...
    เพิ่มประมาณการขายใหม่ Status <kbd
        style="background-color: #EBE4ED; width: 10px; max-height: 10px; border-radius: 0px; border:1px solid #202020;">&nbsp;</kbd>
    งานที่ plan ไว้แล้ว
    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
</div>
<?php } else { ?>
<div style="position: fixed; top:30px; right: 20px; " class="alert alert-danger alert-dismissible fade show"
    role="alert">
...
<form action="daily_report_save" method="post" enctype="multipart/form-data">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 16px;" class="mt-4">
        <span></span>
        <span>
            <label for="proceed1" class="badge rounded-pill"
                style="background-color: #19D700; color:#FFFFFF; padding-left: 15px; padding-right: 15px; margin-right: 10px; display: flex; align-items: center; cursor: pointer;"><img
                    src="assets/images/icon_system/icon-park--save-one.png"
                    style="width:15px; height:15px; color:#FFFFFF;"> &nbsp; บันทึก</label>
            <input type="submit" value="บันทึก" id="proceed1" name="proceed1" style="display: none;">
        </span>
        <?php if ($_SESSION['typelogin'] == 'Supervisor') { ?>
        <span class="badge rounded-pill"
            style="background-color: #FF0004; color:#FFFFFF; padding-left: 15px; padding-right: 15px; display: flex; align-items: center; cursor: pointer;"
            onclick="deletePlan(<?php if (isset($id_work)) {
                                        echo $id_work;
                                    } ?>);" data-bs-toggle="tooltip" data-bs-title="ลบแผลนงาน"><img
                src="assets/images/icon_system/trash-alt-solid-24.png" style="width:15px; height:15px;"> &nbsp;
            Delete</span>
        <?php } else { ?>
        <span></span>
        <?php } ?>
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            Swal.fire({
                ...
                // Optionally, you can pass the textarea value (result.value) to the URL or handle it as needed
                window.location.href =
                    `daily_report_edit?id_work=${idCopy}&dc=1&plan_copynew=${encodeURIComponent(result.value)}`;
            });
        }
    });
}
...
function addProductRow(rowNum, fieldName, searchTerm, txtHint, product_twolist) {
    ...
    xhr.open("GET", "./src/controllers/product_list_controllers.php?q=" + encodeURIComponent(searchTerm) + "&rowNum=" +
        rowNum + "&fieldName=" + encodeURIComponent(fieldName) + "&txtHint=" + encodeURIComponent(txtHint) +
        "&product_twolist=" + encodeURIComponent(product_twolist), true);
    xhr.send();
}
// รายการสินค้า END
</script>
